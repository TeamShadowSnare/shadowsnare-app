"""
User Mode view (PyQt6).
Guides the user through the flow: Create dump → Extract CSV → Analyze.
Displays an HTML summary and supports an 'Explainability' popup dialog.
...
class UserMode(QWidget):
    """UI for end-to-end user workflow (dump, extract, analyze)."""
    def __init__(self):
        super().__init__()

        # State managed by controller; used to enable/disable actions if needed
        ...
        self.arrow_labels = [] # holds the arrow labels between step buttons

        # Instructions (HTML) shown at the top
    ...
    """
        ...
        # Horizontal row that holds the three action buttons and arrows
        row = QHBoxLayout()
        row.setSpacing(16)
        row.setAlignment(Qt.AlignmentFlag.AlignCenter)

        def add_flow_button(btn, is_last=False):
            """Inline helper to add a button and, if needed, an arrow after it."""
            row.addWidget(btn)
            if not is_last:
                arrow = QLabel("➜")
        ...
        # Analysis section (hidden until we have content to show)
        ...
        # Summary container + HTML viewer (right panel content area style)
        self.summary_container = QFrame()
        self.summary_layout = QVBoxLayout(self.summary_container)

        self.data_display = QTextBrowser() # accepts setHtml()
        self.data_display.setReadOnly(True)
        self.data_display.setOpenExternalLinks(False) # handle links manually
        ...
        # Explainability text (filled by controller), and optional raw data area
        ...
        # Prepare the explainability popup dialog (opened via link)
        self.setup_explanation_popup()

    def _button_style(self, color, hover_color):
        """Return a small QSS template for step buttons."""
        return f"""
            QPushButton {{
                font-size: 20px;
                padding: 12px 28px;
    ...
    def setup_connections(self, controller):
        """Connect step buttons to controller slots (no logic here)."""
    ...
    def show_explanation_popup(self):
        """Open the modal and load current explanations into the popup."""
    ...
    def _handle_anchor_click(self, link):
        """Handle clicks from summary HTML (e.g., a '#'-href to show the popup)."""
