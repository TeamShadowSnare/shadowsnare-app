...
#     finished = pyqtSignal(str)  # emits: dump_path
...
#             result = subprocess.run([self.winpmem_path, self.dump_path], check=False)

#             if result.returncode in [0, 1] and os.path.exists(self.dump_path) and os.path.getsize(self.dump_path) > 100 * 1024 * 1024:
...
#             self.error.emit(f"❌ Dump creation failed: {e}")
